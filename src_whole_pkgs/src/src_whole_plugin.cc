...
#include <src_whole_pkgs/Kubot_control_msgs.h>
...
namespace gazebo {
    ...
    class src_whole_plugin : public ModelPlugin
    {
...
//구조체는 변수 집합이라고 생각하면 편하다.
    ...
    public :
        ...
        void Kubot_control_callback(const src_whole_pkgs::Kubot_control_msgs &msg);
    ...
    GZ_REGISTER_MODEL_PLUGIN(src_whole_plugin);
...
void gazebo::src_whole_plugin::Load(physics::ModelPtr _model, sdf::ElementPtr /*_sdf*/)
{
    //Kubot.ControlMode = CTRLMODE_WALKREADY; // wave 함수 초기

    // Kubot.Move_current = false;
    printf("\nLoad_do\n");
    KubotModesp = nh.subscribe("/KubotMode",1,&gazebo::src_whole_plugin::KubotMode,this);
    //0626
    Kubot_control_mode_sub = nh.subscribe("/Kubot_Control_Msg",1,&gazebo::src_whole_plugin::Kubot_control_callback,this);
    Kubot_joy_sub = nh.subscribe("/joy",1,&gazebo::src_whole_plugin::Kubot_joy_callback,this);
    ...
    update_connection = event::Events::ConnectWorldUpdateBegin(boost::bind(&src_whole_plugin::UpdateAlgorithm, this));

}
void gazebo::src_whole_plugin::Kubot_joy_callback(const sensor_msgs::Joy &joy_msgs){
    printf("stamp : %d\n",joy_msgs.header.seq);
    printf("joy_data[0] :%f\n",joy_msgs.axes[0]);
    printf("joy_data[1] :%f\n",joy_msgs.axes[1]);
    printf("joy_data[2] :%f\n",joy_msgs.axes[2]);
...
void gazebo::src_whole_plugin::KubotMode(const std_msgs::Int32 &msg)
{
    ...
    case 24:
        Kubot.startTime = Kubot.realTime;
        Kubot.ControlMode = CTRLMODE_MOTION;
        Kubot.CommandFlag = ACT_KICKING;
        printf("KICKING START! \n");
...
void gazebo::src_whole_plugin::Kubot_control_callback(const src_whole_pkgs::Kubot_control_msgs &msg){
    int Kubot_mode_msgs;
...
void gazebo::src_whole_plugin::UpdateAlgorithm()
{
...
// } amp 목표값-현재값, 전체 시간, dt같은 느낌, 현재값
            ...
            // else if (Kubot.Move_current == true && Kubot.CommandFlag == ACT_SWING_ARMS) {

            //     // double CKubot::cosWave(amp,  period,  time,  int_pos)
            //     // {
            //     //    return (amp / 2)*(1 - cos(PI / period * time)) + int_pos;
            //     // } amp 목표값-현재값, 전체 시간, dt같은 느낌, 현재값
            //     double swingAngle[2]; //Base 위치

            //     double swingAngle_max = -30 * D2R;
            //     double swingAngle_min = 30 * D2R;
            //     double T = 1000;

            //     swingAngle[0] = Kubot.cosWave(swingAngle_max-swingAngle_min, T, Kubot.updown_cnt, swingAngle_min); // L_
            //     swingAngle[1] = Kubot.cosWave(swingAngle_max-swingAngle_min, T, Kubot.updown_cnt, swingAngle_min); // R_
            ...
            //     Kubot.refAngle[12] = swingAngle[0];
            //     Kubot.refAngle[15] = swingAngle[1];
...
void gazebo::src_whole_plugin::setjoints() //plugin에다가 joints name 설정 .sdf파일에서 설정한 이름이랑 확인하기
{
    /*
    ...
    L_Ankle_pitch_joint = this->model->GetJoint("LAP");
    L_Ankle_roll_joint = this->model->GetJoint("LAR");
    ...
    R_Ankle_pitch_joint = this->model->GetJoint("RAP");
    R_Ankle_roll_joint = this->model->GetJoint("RAR");

    L_shoulder_pitch_joint = this->model->GetJoint("LSHP");
    L_elbow_roll_joint = this->model->GetJoint("LSHR");
    L_hand_pitch_joint = this->model->GetJoint("LE");

    R_shoulder_pitch_joint = this->model->GetJoint("RSHP");
    R_elbow_roll_joint = this->model->GetJoint("RSHR");
    R_hand_pitch_joint = this->model->GetJoint("RE");

    Neck_yaw_joint = this->model->GetJoint("NY");
    head_pitch_joint = this->model->GetJoint("NP");

    //L_Foot_joint = this->model->GetJoint("L_Foot_joint");
    //R_Foot_joint = this->model->GetJoint("R_Foot_joint");
}

void gazebo::src_whole_plugin::getjointdata()
{
    /*
...
void gazebo::src_whole_plugin::setsensor() //sdf파일에 있는 sensor를 설정하는 함수
{
        Sensor = sensors::get_sensor("IMU");
        IMU = std::dynamic_pointer_cast<sensors::ImuSensor>(Sensor);   
}

void gazebo::src_whole_plugin::getsensordata()
{
     // IMU sensor data
        double IMUdata[3];
...
void gazebo::src_whole_plugin::jointcontroller()
{
    // double amp = 1;
...
void gazebo::src_whole_plugin::initializejoint()
{
    /*
...
void gazebo::src_whole_plugin::setjointPIDgain()
{
    /*
...
void gazebo::src_whole_plugin::InitRosTopic()
{
    //ROS Publishers
    KubotModesp_pub = nh.advertise<std_msgs::Float64>("command/KubotMode", 1000);
...
void gazebo::src_whole_plugin::RosTopicPublish()
{
    //////////////base//////////////
    m_preview_COM_y.data = Kubot.zmp.previewControl.Y.CoM;
    m_preview_ref_ZMP_y.data = Kubot.zmp.previewControl.Y.m_ref;
