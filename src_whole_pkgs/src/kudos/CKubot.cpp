...
void CKubot::initializeBkubot() {
    ...
    body_kine.lower.BASE_TO_LEG_Z = 115.68; // mm
    body_kine.lower.THIGH_LENGTH = 120.0; //
    body_kine.lower.CALF_LENGTH = 120.0; //mm
    body_kine.lower.ANKLE_LENGTH = 50; //mm
    ...
    body_kine.lower.foot_x_length = 140; // mm
...
void CKubot::setZmpPreview(double previewTime){
    ...
    zmp.previewControl.C << 1 ,0 ,-(0.34*1.15)/9.81;
...
void CKubot::setZmpPreviewTime(double previewTime){
    ...
    zmp.previewControl.RefTotalTrajSize = (zmp.previewControl.PreviewReadyTime\
                                           + walking.step.total_num * walking.time.periodTime\
...
//            printf("=========Walking========\n");
    ...
    else if(controlMODE == CTRLMODE_WALKING_TEST)
    {
        ...
        else if(CommandFLAG == ACT_START_WALK)  //TURN++
        {
            ...
            initializeGlobalStepPosition(); 
            walking.initialize = true; 
            setWalkingStep(7,walking.step.S_L_F,0,0,0,0.025); //0.05 
            setWalkingTime(1.0,0.2);
            setZmpPreview(1.5);
            FootStepsPlanner();
            generateZmpTraj(walking.step.LFoot,walking.step.RFoot);
        ...
        // else if(CommandFLAG == ACT_STOPPING_PATTERN_WALK)  //TURN++
        // {
        //     printf("========= INF walking PatternGenerator check ========\n");

        //     initializeGlobalStepPosition(); 
        //     walking.initialize = true; 
        //     setWalkingStep(7,walking.step.S_L_F,0,0,0,0.02); //0.05 
        //     setWalkingTime(1.0,0.2);
        //     setZmpPreview(1.5);
        //     FootStepsPlanner();
        //     generateZmpTraj(walking.step.LFoot,walking.step.RFoot);
...
void CKubot::FootStepsPlanner() { // turn관련 식만 주석처리하고 사용
    ...
    if (walking.step.total_footprints % 2 == 0) { //hooooo //발자국 짝수 보행 홀수
    ...
    for (int m_step = 0; m_step < walking.step.total_footprints; m_step++) { //hooooo //m_step : 발자국 개수 시작발왼발 -> 오른발 지지
        if (walking.step.start_swing_foot == walking.step.S_L_F) {
            if (m_step == walking.step.total_footprints - 1) {
                if (walking.step.total_footprints % 2 == 1) {// 왼쪽발과 위치를 나란히 놓겠다.
                    walking.step.RFoot(X, m_step / 2) = (1 - walking.step.walking_Turn)*(walking.step.LFoot(X, m_step / 2 - 1))\
                                                   + (walking.step.walking_Turn)*((body_kine.lower.R.Base_to_Foot(Y) - walking.step.turningPoint) * sin((m_step - 1)*-walking.step.TurnSize));
                    walking.step.RFoot(Y, m_step / 2) = (1 - walking.step.walking_Turn)*(walking.step.LFoot(Y, m_step / 2 - 1) + 2*body_kine.lower.R.Base_to_Foot(Y))\
                    ...
                    walking.step.LFoot(X, (int) ((float) (m_step) / 2.)) = (1 - walking.step.walking_Turn) * walking.step.RFoot(X, (int) ((float) (m_step) / 2.))\
                                                                     + (walking.step.walking_Turn)*((body_kine.lower.L.Base_to_Foot(Y) - walking.step.turningPoint) * sin((m_step - 1)*-walking.step.TurnSize));
                    walking.step.LFoot(Y, (int) ((float) (m_step) / 2.)) = (1 - walking.step.walking_Turn)*(((int) ((float) (m_step) / 2.)) * walking.step.LRstepSize + body_kine.lower.L.Base_to_Foot(Y))\
                    ...
                    walking.step.RFoot(X, m_step / 2) = (1 - walking.step.walking_Turn)*(walking.step.LFoot(X, m_step / 2))\
                                                   + (walking.step.walking_Turn)*((body_kine.lower.R.Base_to_Foot(Y) - walking.step.turningPoint) * sin((m_step - 1)*-walking.step.TurnSize));
                    walking.step.RFoot(Y, m_step / 2) = (1 - walking.step.walking_Turn)*(walking.step.LFoot(Y, m_step / 2) + 2 * body_kine.lower.R.Base_to_Foot(Y))\
                                                   + (walking.step.walking_Turn)*((body_kine.lower.R.Base_to_Foot(Y) - walking.step.turningPoint) * cos((m_step - 1)*-walking.step.TurnSize) + walking.step.turningPoint);
                ...
                if (m_step == 0) { //짝수 1 2 3 발자국  0 1 2 m_step
                    walking.step.RFoot(X, m_step / 2) = 0;
                    walking.step.RFoot(Y, m_step / 2) = (1 - walking.step.walking_Turn)*((m_step / 2) * walking.step.LRstepSize + body_kine.lower.R.Base_to_Foot(Y))\
                                                      + (walking.step.walking_Turn)*((body_kine.lower.R.Base_to_Foot(Y) - walking.step.turningPoint) * cos((m_step)*-walking.step.TurnSize) + walking.step.turningPoint);
                    ...
                    walking.step.RFoot(X, m_step / 2) = (1 - walking.step.walking_Turn)*((m_step) * walking.step.FBstepSize)\
                                                      + (walking.step.walking_Turn)*((body_kine.lower.R.Base_to_Foot(Y) - walking.step.turningPoint) * sin((m_step)*-walking.step.TurnSize));
                    walking.step.RFoot(Y, m_step / 2) = (1 - walking.step.walking_Turn)*((m_step / 2) * walking.step.LRstepSize + body_kine.lower.R.Base_to_Foot(Y))\
                                                      + (walking.step.walking_Turn)*((body_kine.lower.R.Base_to_Foot(Y) - walking.step.turningPoint) * cos((m_step)*-walking.step.TurnSize) + walking.step.turningPoint) + Y_step_position;
                    walking.step.RFoot(Yaw, m_step / 2) = (m_step) * walking.step.TurnSize;
                }
            } else { // 홀수 float0.5를 int로 때리면 0으로 만들고 행렬 속으로 집어넣어

                walking.step.LFoot(X, (int) ((float) (m_step) / 2.)) = (1 - walking.step.walking_Turn)*((m_step) * walking.step.FBstepSize)\
                                                                 + (walking.step.walking_Turn)*((body_kine.lower.L.Base_to_Foot(Y) - walking.step.turningPoint) * sin((m_step)*-walking.step.TurnSize));
                walking.step.LFoot(Y, (int) ((float) (m_step) / 2.)) = (1 - walking.step.walking_Turn)*(((int) ((float) (m_step) / 2.) + 1) * walking.step.LRstepSize + body_kine.lower.L.Base_to_Foot(Y))\
                                                                 + (walking.step.walking_Turn)*((body_kine.lower.L.Base_to_Foot(Y) - walking.step.turningPoint) * cos((m_step)*-walking.step.TurnSize) + walking.step.turningPoint) - Y_step_position;
            ...
            if (m_step == walking.step.total_footprints - 1) {
                if (walking.step.total_footprints % 2 == 1) {
                    walking.step.LFoot(X, m_step / 2) = (1 - walking.step.walking_Turn)*(walking.step.RFoot(X, m_step / 2 - 1))\
                                                     + (walking.step.walking_Turn)*((body_kine.lower.L.Base_to_Foot(Y) - walking.step.turningPoint) * sin((m_step - 1)*-walking.step.TurnSize));
                    walking.step.LFoot(Y, m_step / 2) = (1 - walking.step.walking_Turn)*(walking.step.RFoot(Y, m_step / 2 - 1) + 2 * body_kine.lower.L.Base_to_Foot(Y))\
                                                     + (walking.step.walking_Turn)*((body_kine.lower.L.Base_to_Foot(Y) - walking.step.turningPoint) * cos((m_step - 1)*-walking.step.TurnSize) + walking.step.turningPoint);
                    walking.step.LFoot(Yaw, m_step / 2) = (m_step - 1) * walking.step.TurnSize;
                } else {
                    walking.step.RFoot(X, (int) ((float) (m_step) / 2.)) = (1 - walking.step.walking_Turn)*(walking.step.LFoot(X, (int) ((float) (m_step) / 2.)))\
                                                                     + (walking.step.walking_Turn)*((body_kine.lower.R.Base_to_Foot(Y) - walking.step.turningPoint) * sin((m_step - 1)*-walking.step.TurnSize));
                    walking.step.RFoot(Y, (int) ((float) (m_step) / 2.)) = (1 - walking.step.walking_Turn)*(((int) ((float) (m_step) / 2.)) * walking.step.LRstepSize + body_kine.lower.R.Base_to_Foot(Y))\
                                                                     + (walking.step.walking_Turn)*((body_kine.lower.R.Base_to_Foot(Y) - walking.step.turningPoint) * cos((m_step - 1)*-walking.step.TurnSize) + walking.step.turningPoint);
                ...
                if (m_step == 0) {
                    ...
                    walking.step.LFoot(Y, m_step / 2) = (1 - walking.step.walking_Turn)*((m_step / 2) * walking.step.LRstepSize + body_kine.lower.L.Base_to_Foot(Y))\
                                                      + (walking.step.walking_Turn)*((body_kine.lower.L.Base_to_Foot(Y) - walking.step.turningPoint) * cos((m_step)*-walking.step.TurnSize) + walking.step.turningPoint);
                ...
                walking.step.RFoot(X, (int) ((float) (m_step) / 2.)) = (1 - walking.step.walking_Turn)*((m_step) * walking.step.FBstepSize)\
                                                                   + (walking.step.walking_Turn)*((body_kine.lower.R.Base_to_Foot(Y) - walking.step.turningPoint) * sin((m_step)*-walking.step.TurnSize));
                walking.step.RFoot(Y, (int) ((float) (m_step) / 2.)) = (1 - walking.step.walking_Turn)*(((int) ((float) (m_step) / 2.) + 1) * walking.step.LRstepSize + body_kine.lower.R.Base_to_Foot(Y))\
                                                                   + (walking.step.walking_Turn)*((body_kine.lower.R.Base_to_Foot(Y) - walking.step.turningPoint) * cos((m_step)*-walking.step.TurnSize) + walking.step.turningPoint) + Y_step_position;
...
void CKubot::generateZmpTraj(MatrixXd LfootPrints, MatrixXd RfootPrints) {
    ...
    for (int pre_cnt = 0; \
            ...
            pre_cnt++) { //hooooo : 전체 사이즈에 변수 넣어서 곱하고 어쩌고 한 값까지 cnt를 생성하는 것.
    ...
    for (int m_step = 0; m_step < walking.step.total_num; m_step++) { //여기서 m_step -> 그냥 step
...
void CKubot::generateFootTraj(MatrixXd LfootPrints, MatrixXd RfootPrints) {
    ...
    if (walking.time.sec < zmp.previewControl.PreviewReadyTime) { // Pattern Phase : DSP
    ...
    //* LfootTraj
        ...
        {                    
            ...
            //    LFoot.refpos(Y)    = cosWave(LfootPrints(Y,LFoot_step_num)-walking.step.LastLfoot(Y), walking.time.XYSSP_time, walking.time.sec-walking.time.XYSSP_start_time, walking.step.LastLfoot(Y)-body_kine.lower.LEG_SIDE_OFFSET);
                LFoot.refpos(Yaw)  = cosWave(LfootPrints(Yaw,LFoot_step_num)-walking.step.LastLfoot(Yaw), walking.time.XYSSP_time, walking.time.init_sec-walking.time.XYSSP_start_time, walking.step.LastLfoot(Yaw));
            ...
            //    LFoot.refpos(Y)   = cosWave(LfootPrints(Y,LFoot_step_num)-LfootPrints(Y,LFoot_step_num-1), walking.time.XYSSP_time, walking.time.sec-walking.time.XYSSP_start_time, LfootPrints(Y,LFoot_step_num-1)-body_kine.lower.LEG_SIDE_OFFSET);
                LFoot.refpos(Yaw) = cosWave(LfootPrints(Yaw,LFoot_step_num)-LfootPrints(Yaw,LFoot_step_num-1), walking.time.XYSSP_time, walking.time.init_sec-walking.time.XYSSP_start_time, LfootPrints(Yaw,LFoot_step_num-1));
...
void CKubot::standupBack(float standupTime)
{
    ...
    else if (STUtime >= 0 && motion_2 == false)
    {
        ...
        standupAngle[12] = -205 * D2R;   // L_shoulder_pitch_joint
        standupAngle[13] = 0 * D2R;  // L_elbow_roll_joint
        standupAngle[14] = 0 * D2R;    // L_hand_pitch_joint
        standupAngle[15] = -205 * D2R;   // R_shoulder_pitch_joint
        standupAngle[16] = -0 * D2R; // R_elbow_roll_joint
        standupAngle[17] = 0 * D2R;    // R_hand_pitch_joint
        standupAngle[18] = 0 * D2R;    // Neck_yaw_joint
        standupAngle[19] = 0 * D2R;    // head_pitch_joint
    ...
    else if (STUtime >= 0 && motion_4 == false)
    {
        ...
        standupAngle[12] = -270 * D2R;   // L_shoulder_pitch_joint
        standupAngle[13] = 0 * D2R;  // L_elbow_roll_joint
        standupAngle[14] = -0 * D2R;    // L_hand_pitch_joint
        standupAngle[15] = -270 * D2R;   // R_shoulder_pitch_joint
        standupAngle[16] = -0 * D2R; // R_elbow_roll_joint
        standupAngle[17] = -0 * D2R;    // R_hand_pitch_joint
        standupAngle[18] = 0 * D2R;    // Neck_yaw_joint
        standupAngle[19] = 0 * D2R;    // head_pitch_joint
    ...
    else if (STUtime >= 0 && motion_5 == false)
    {
        ...
        standupAngle[12] = -240 * D2R;   // L_shoulder_pitch_joint
        standupAngle[13] = 0 * D2R;  // L_elbow_roll_joint
        standupAngle[14] = -0 * D2R;    // L_hand_pitch_joint
        standupAngle[15] = -240 * D2R;   // R_shoulder_pitch_joint
        standupAngle[16] = -0 * D2R; // R_elbow_roll_joint
        standupAngle[17] = -0 * D2R;    // R_hand_pitch_joint
        standupAngle[18] = 0 * D2R;    // Neck_yaw_joint
        standupAngle[19] = 0 * D2R;    // head_pitch_joint
...
void CKubot::hurdle(float hurdleTime){
    ...
    if (HDLtime >= 0 && standup_1 == false){   // zmp 이동

        GBS_Vector << 0, 0.06, 0.38, 0, 0, 0;
    ...
    else if (HDLtime >= 0 && standup_2 == false){   // 발 앞으로 들기

        GBS_Vector << 0, 0.06, 0.38, 0, 0, 0;
        GFR_Vector << 0.04, -0.05, 0.1, 0, 0, 0;
        GFL_Vector << 0, 0.05, 0, 0, 0, 0;
    ...
    else if (HDLtime >= 0 && standup_3 == false){   // 발 앞으로 내리기

        GBS_Vector << 0, 0.06, 0.38, 0, 0, 0;
        GFR_Vector << 0.08, -0.05, 0, 0, 0, 0;
        GFL_Vector << 0, 0.05, 0, 0, 0, 0;
    ...
    else if (HDLtime >= 0 && standup_4 == false){   // zmp 이동

        GBS_Vector << 0.08, -0.06, 0.38, 0, 0, 0;
        GFR_Vector << 0.08, -0.05, 0, 0, 0, 0;
        GFL_Vector << 0, 0.05, 0, 0, 0, 0;
    ...
    else if (HDLtime >= 0 && standup_5 == false){   // 발 들면서 앞으로

        GBS_Vector << 0.08, -0.06, 0.38, 0, 0, 0;
        GFR_Vector << 0.08, -0.05, 0, 0, 0, 0;
        GFL_Vector << 0.04, 0.05, 0.1, 0, 0, 0;
...
///////////////////////////////////////////////////////////
    else if (HDLtime >= 0 && standup_6 == false){   // 발 내리면서 ready pose로

        GBS_Vector << 0.08, 0, 0.38, 0, 0, 0;
        GFR_Vector << 0.08, -0.05, 0, 0, 0, 0;
        GFL_Vector << 0.08, 0.05, 0, 0, 0, 0;
...
void CKubot::kick_process(float kickTime){

    // kickTime = 1.0
    static float KICKtime = 0;
    static float currentAngle[20]; // Now nDoF = 20
    VectorXd GBS_Vector(6);
    VectorXd GFL_Vector(6);
    VectorXd GFR_Vector(6);

    VectorXd IK_kickAngle(12);

    if (KICKtime >= 0 && standup_1 == false){   // zmp 이동

        GBS_Vector << 0, 0.06, 0.38, 0, 0, 0;
        GFR_Vector << 0, -0.05, 0, 0, 0, 0;
        GFL_Vector << 0, 0.05, 0, 0, 0, 0;

        IK_kickAngle << Geometric_IK_L(GBS_Vector, GFL_Vector),Geometric_IK_R(GBS_Vector, GFR_Vector);

        for (int j = 0; j < 12; j++) {
                kickAngle[j] = IK_kickAngle(j);
            }

            kickAngle[12] = (30 *D2R);     // L_shoulder_pitch_joint 플러스가 어깨 뒤로
            kickAngle[13] = (15 *D2R);     // L_elbow_roll_joint 플러스가 좌우로 나란히
            kickAngle[14] = (-130 *D2R);     // L_hand_pitch_joint  플러스가 안으로 굽
            kickAngle[15] = (30  *D2R);     // R_shoulder_pitch_joint
            kickAngle[16] = (-15 *D2R);     // R_elbow_roll_joint
            kickAngle[17] = (-130 *D2R);     // R_hand_pitch_joint
            kickAngle[18] = (0 *D2R);     // Neck_yaw_joint
            kickAngle[19] = (0 *D2R);     // head_pitch_joint


        if (KICKtime == 0 && standup_1 == false) {
        ...
        if (KICKtime < kickTime && standup_1 == false) {
            for (int j = 0; j < 20; j++) {
                refAngle[j] = cosWave(kickAngle[j]- currentAngle[j], kickTime, KICKtime, currentAngle[j]);
            }
            KICKtime += tasktime;
        }
    ...
    else if (KICKtime >= 0 && standup_2 == false){   // 발 뒤로 들기

        GBS_Vector << 0, 0.06, 0.38, 0, 0, 0;
        GFR_Vector << -0.03, -0.05, 0.06, 0, 0, 0;
        GFL_Vector << 0, 0.05, 0, 0, 0, 0;
        ...
        if (KICKtime < kickTime && standup_2 == false) {
            for (int j = 0; j < 20; j++) {
                refAngle[j] = cosWave(kickAngle[j]- currentAngle[j], kickTime, KICKtime, currentAngle[j]);
            }
            KICKtime += tasktime;
    ...
    else if (KICKtime >= 0 && standup_3 == false){   // 공 차기

        GBS_Vector << 0, 0.06, 0.38, 0, 0, 0;
        GFR_Vector << 0.07, -0.05, 0.06, 0, 0, 0;
        GFL_Vector << 0, 0.05, 0, 0, 0, 0;
        ...
        if (KICKtime < kickTime && standup_3 == false) {
            for (int j = 0; j < 20; j++) {
                refAngle[j] = cosWave(kickAngle[j]- currentAngle[j], kickTime, KICKtime, currentAngle[j]);
            }
            KICKtime += tasktime;
    ...
    else if (KICKtime >= 0 && standup_4 == false){   // 발 내리기
        ...
        if (KICKtime < kickTime && standup_4 == false) {
            for (int j = 0; j < 20; j++) {
                refAngle[j] = cosWave(kickAngle[j]- currentAngle[j], kickTime, KICKtime, currentAngle[j]);
            }
            KICKtime += tasktime;
    ...
    else if (KICKtime >= 0 && standup_5 == false){   // zmp 제자리

        GBS_Vector << 0, 0, 0.38, 0, 0, 0;
        GFR_Vector << 0, -0.05, 0, 0, 0, 0;
        GFL_Vector << 0, 0.05, 0, 0, 0, 0;
        ...
        if (KICKtime < kickTime && standup_5 == false) {
            for (int j = 0; j < 20; j++) {
                refAngle[j] = cosWave(kickAngle[j]- currentAngle[j], kickTime, KICKtime, currentAngle[j]);
            }
            KICKtime += tasktime;
...
void CKubot::BallSwingMotion(float upTime, float swingTime)
{
    static float STUtime = 0;
    static float currentAngle[20]; // Now nDoF = 20
    ...
    else if (STUtime >= 0 && motion_3 == false)
    {
        ...
        standupAngle[13] = (0 * D2R);   // L_elbow_roll_joint 플러스가 좌우로 나란히
        standupAngle[14] = (-85 * D2R); // L_hand_pitch_joint  플러스가 안으로 굽
        standupAngle[15] = (-220 * D2R); // R_shoulder_pitch_joint
        standupAngle[16] = (-0 * D2R);  // R_elbow_roll_joint
    ...
    else if (STUtime >= 0 && motion_4 == false)
    {
        standupAngle[0] =        -0 * D2R;        // L_Hip_yaw_joint
        standupAngle[1] =         0 * D2R;        // L_Hip_roll_joint
        standupAngle[2] =  -35.4901 * D2R;         // L_Hip_pitch_joint
        standupAngle[3] =   24.9802 * D2R;         // L_Knee_joint
        standupAngle[4] =  -12.4901 * D2R;          // L_Ankle_pitch_joint
        standupAngle[5] =         0 * D2R;        // L_Ankle_roll_joint
        standupAngle[6] =        -0 * D2R;        // R_Hip_yaw_joint
        standupAngle[7] =         0 * D2R;        // R_Hip_roll_joint
        standupAngle[8] =  -35.4901 * D2R;         // R_Hip_pitch_joint
        standupAngle[9] =   24.9802 * D2R;         // R_Knee_joint
        standupAngle[10] = -12.4901 * D2R;           // R_Ankle_pitch_joint
        standupAngle[11] =        0 * D2R;         // R_Ankle_roll_joint
        ...
        if (STUtime < swingTime && motion_4 == false)
        {
            for (int j = 0; j < 20; j++)
            {
                refAngle[j] = cosWave(standupAngle[j] - currentAngle[j], swingTime, STUtime, currentAngle[j]);
            }
            STUtime += tasktime;
        }
