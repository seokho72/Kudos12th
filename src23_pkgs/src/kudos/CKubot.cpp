...
MatrixXd CKubot::jointToTransform01(VectorXd q)
{
    ...
    tmp_m << cos(qq), -sin(qq), 0, 0.007455,
...
{                   // Global to Base,Foot position and orientaion
    ...
    BH << 0.007455, 0.05, -0.18676; // Base to Hip

    R1 = GB.block(0, 0, 3, 3);
    R7 = GF.block(0, 0, 3, 3);
    pf = GF.block(0, 3, 3, 1); // Foot position
    FA << 0, 0, 0.05;          // Foot to Ankle
    p7 = pf + FA;
...
void CKubot::IK()
{
    ...
    GB_PosOri << 0, 0, 0.43, 0, 0, 0;
    GF_PosOri_L << 0.007455, 0.05, -0.18676, 0, 0, 0;
    GF_PosOri_R << 0.007455, -0.05, -0.18676, 0, 0, 0;
...
void CKubot::initializeBkubot()
{
    ...
    body_kine.lower.BASE_TO_LEG_Z = 0.18676;                                                                                // 104.64; // mm
    body_kine.lower.THIGH_LENGTH = 0.120;                                                                                   // 138.0; //
    body_kine.lower.CALF_LENGTH = 0.120;                                                                                    // 143.0; //mm
    body_kine.lower.ANKLE_LENGTH = 0.05;                                                                                    // 50; //mm
    ...
    body_kine.lower.foot_x_length = 0.15; // 150; // mm
    body_kine.lower.foot_y_length = 0.10; // 100; // mm
...
void CKubot::setZmpPreview(double previewTime)
{
    ...
    zmp.previewControl.C << 1, 0, -(0.43 * 0.6) / 9.81; // 0.43
...
void CKubot::setZmpPreviewTime(double previewTime)
{
    ...
    zmp.previewControl.PreviewTime = previewTime; // 0.6
    zmp.previewControl.PreviewReadyTime = 1.2;    // 1.3;    // 2.0;  //  period *4/3

    zmp.previewControl.PreviewTimeSize = zmp.previewControl.PreviewTime * (double)onesecSize; // 1.2 * 1000  == 1200
    zmp.previewControl.PeriodTimeSize = walking.time.periodTime * (double)onesecSize;         // 0.6 * 1000 == 600

    zmp.previewControl.RefTotalTrajSize = (zmp.previewControl.PreviewReadyTime + walking.step.total_num * walking.time.periodTime + 3 * zmp.previewControl.PreviewTime) * (double)onesecSize; // (0.8 + 7*0.6 + 3*0.6)*1000 == (0.8+4.2+1.8)*1000 == 6800

    zmp.previewControl.RefTotalTime = zmp.previewControl.RefTotalTrajSize / (double)onesecSize; // 6800/1000 == 6.9
...
void CKubot::walkingPatternGenerator(int controlMODE, int CommandFLAG, double steps, double FBsize, double LRsize, double Turnsize, double footHeight, int stop)
{
    ...
    else if (controlMODE == CTRLMODE_WALKING_TEST)
    {
        ...
        else if (CommandFLAG == ACT_START_WALK) // TURN++
        {
            ...
            initializeGlobalStepPosition();
            walking.initialize = true;
            setWalkingStep(7, walking.step.S_L_F, 0, 0, 0, 0.01); // 0.05 / 0.02
            setWalkingTime(0.6, 0.1);                             // 1.0, 0.2  // period 1.0 일 때, previewTime 1.5  (1.5배..?)
            setZmpPreview(1.5);                                   //  period * 1.5
            FootStepsPlanner();
            generateZmpTraj(walking.step.LFoot, walking.step.RFoot);
            walking.walk = true;
        ...
        else if (CommandFLAG == ACT_TEST_WALK || CommandFLAG == ACT_INF_WALK || CommandFLAG == ACT_STOPPING_WALK || CommandFLAG == ACT_STOP_WALK)
        {
            ...
            walking.time.sec = (float)(zmp.previewControl.count) * (float)(tasktime); // --> 5000 * 0.001 == 5.0
            // printf("walking.time.sec =%lf\n",walking.time.sec );
            // printf("walk_time_start : % lf\n",walking.time.XYSSP_start_time);
            // printf("walk_time_end : % lf\n",walking.time.XYSSP_end_time);
            // printf("walk_time_init_time : % lf\n",walking.time.init_sec);

            // printf("double : %lf, int : %d\n", walking.time.sec ,(int)walking.time.sec );

            // if(walking.time.sec - (int)walking.time.sec -0.001 <= 0.0){
            //     // printf("init_comp");
            //     walking.time.old_sec = walking.time.sec;
            // }

            if (fmodf(walking.time.sec, walking.time.periodTime) == 0.0)
            { // time.sec = 5.0 이후, periodTime의 배수  --> 6.0  --> old_sec == 6.0 되야하지 않나?
                // printf("init_comp");

                walking.time.old_sec = walking.time.sec;
            }
            walking.time.init_sec = walking.time.sec - walking.time.old_sec; // -0.5 period 때문에 보정해줌

            ///////////////////////////////////////////////////////////////////////////
            ///////////////////////////////////////////////////////////////////////////
            // if (walking.time.sec >= 0.6 && walking.time.sec < 1.2)
            // {
            //     if (fmodf(walking.time.sec, walking.time.periodTime) == 0.0)
            //     {
            //         walking.time.old_sec = walking.time.sec;
            ...
            //     walking.time.init_sec = walking.time.sec - walking.time.old_sec;

            // }

            // else if (walking.time.sec >= 1.2 && walking.time.sec < 2.4)
            // {
            //     // walking.time.old_sec = 0;

            //     if (fmodf(walking.time.sec, walking.time.periodTime * 2) == 0.0)
            //     {
            //         walking.time.old_sec = walking.time.sec;
            //     }
            ...
            // printf("\n walking.time.periodTime =%lf\n", walking.time.periodTime );
            // printf("\n walking.time.sec =%lf\n",walking.time.sec );
            // printf("walking.time.old_sec =%lf\n", walking.time.old_sec );
            // // // printf("walk_time_start : % lf\n",walking.time.XYSSP_start_time);
            // // // printf("walk_time_end : % lf\n",walking.time.XYSSP_end_time);
            // printf("walk_time_init_time : % lf\n",walking.time.init_sec);
            // printf("walk_time_wsec : % lf\n",walking.time.wsec);
...
void CKubot::setWalkingTime(double periodTime, double DSPratio)
{
    ...
    walking.time.periodTime = periodTime;                // 1.5 / 0.6
    walking.time.DSP_ratio = DSPratio;                   // 0.2 / 0.4
    walking.time.SSP_ratio = 1 - walking.time.DSP_ratio; // 0.6

    // printf(C_CYAN "Walking periodTime = %lf sec\t DSP %lf\t SSP %lf\n" C_RESET, periodTime, walking.time.DSP_ratio, walking.time.SSP_ratio);

    walking.time.SSP_start_time = walking.time.periodTime * walking.time.DSP_ratio / 2; // 0.6*0.4/2   == 0.12
    walking.time.SSP_time = walking.time.periodTime * walking.time.SSP_ratio;           // 0.6*0.6   == 0.36  // 0.5*0.5
    walking.time.SSP_end_time = walking.time.SSP_start_time + walking.time.SSP_time;    // 0.36+0.12     == 0.48

    // walking.time.XYDSP_ratio = 1.2 * DSPratio;  //  1.2 / 0.6
...
{ // turn관련 식만 주석처리하고 사용
    ...
    {                                                                                           // hooooo //발자국 짝수 보행 홀수
        ...
        walking.step.RFoot = MatrixXd::Zero(PatternElement, walking.step.total_footprints / 2); // 각 발에 4개의 발자국씩 생성됨
    }
    else
    {
    ...
    { // hooooo //m_step : 발자국 개수 시작발왼발 -> 오른발 지지
        ...
        if (walking.step.start_swing_foot == walking.step.S_L_F)
        {
            if (m_step == walking.step.total_footprints - 1) // 마지막 스텝
            {
    ...
    std::cout << "LFoot : " << walking.step.LFoot << std::endl;
    std::cout << "RFoot : " << walking.step.RFoot << std::endl;
}
...
void CKubot::generateZmpTraj(MatrixXd LfootPrints, MatrixXd RfootPrints)
{
    ...
    static double zmp_y_offset = -0.01; // 0.065 * 0;
    ...
    { // 여기서 m_step -> 그냥 step
        ...
        if (walking.step.start_swing_foot == walking.step.S_L_F) // 우리는 SLF
        {
            if ((m_step) % 2 == 0) //
            {
                ...
                for (int pre_cnt = zmp.previewControl.RefTotalTrajSize * (zmp.previewControl.PreviewReadyTime + (m_step)*walking.time.periodTime) / zmp.previewControl.RefTotalTime;
                     ...
                     pre_cnt++) // for(pre_cnt = 1000*(2.0 + 짝수*1.0); pre_cnt < 1000*(2.0 + (짝수+1)*1.0); pre_cnt++)   --> periodTime 동안 즉 한 스텝 동안 베이스 움직임
                {
...
void CKubot::generateFootTraj(MatrixXd LfootPrints, MatrixXd RfootPrints)
{
    ...
    // double hip_compensate = 1.75*PI/180.0;// 2.0*PI/180.0; //onlyzmpcontrol 1.75*PI/180.0
    ...
    static double L_offset_Z = 0;
    static double R_offset_Z = 0;
    static double d_L_offset_Z = 0;
    static double d_R_offset_Z = 0;
    ...
    {
        predicted_walking_time = fmodf(walking.time.sec - zmp.previewControl.PreviewReadyTime, walking.time.periodTime); // fmodf : 실수형 나머지  // fmodf(4.999-0.8 , 0.6) == 0.599..?
        // printf("\n predicted_walking_time : %f \n", predicted_walking_time);
        predicted_walking_step_num = (int)((walking.time.sec - zmp.previewControl.PreviewReadyTime) / walking.time.periodTime + 0.0001);
        // printf("\n predicted_walking_step_num : %d \n", predicted_walking_step_num);  // 2 여야한다.
        ...
        // if (walking.time.sec >= 0 && walking.time.sec < 0.6){  // step_num == 0
        // predicted_walking_step_num = (int) ((walking.time.sec - zmp.previewControl.PreviewReadyTime / walking.time.periodTime + 0.0001));
        // }

        // else if (walking.time.sec >= 0.6 && walking.time.sec < 2.4){    // step_num == 1
        // predicted_walking_step_num = (int) ((walking.time.sec - zmp.previewControl.PreviewReadyTime / walking.time.periodTime + 1.0001));
        // }

        // else if (walking.time.sec >= 2.4){  // step_num == 2
        // predicted_walking_step_num = (int) ((walking.time.sec - zmp.previewControl.PreviewReadyTime / walking.time.periodTime + 2.0001));
        // }
        // printf("\n predicted_walking_step_num : %d \n", predicted_walking_step_num);  // 2 여야한다.
        /////////////////////////////////////////////////////////////////////////////////////
        /////////////////////////////////////////////////////////////////////////////////////
    ...
    else if (walking.Pattern_SP == walking.RSSP)
    {
        ...
        if (walking.time.init_sec < walking.time.SSP_start_time) // 0.12
        {
        }
        else if (walking.time.init_sec < walking.time.SSP_end_time) // 0.48
        {
            ...
            if (LFoot_step_num == 0)
            {
                ...
                LFoot.refpos(Yaw) = cosWave(LfootPrints(Yaw, LFoot_step_num) - walking.step.LastLfoot(Yaw), walking.time.SSP_time, walking.time.init_sec - walking.time.SSP_start_time, walking.step.LastLfoot(Yaw)); // init_sec
            ...
            else
            {
                ...
                LFoot.refpos(Yaw) = cosWave(LfootPrints(Yaw, LFoot_step_num) - LfootPrints(Yaw, LFoot_step_num - 1), walking.time.SSP_time, walking.time.init_sec - walking.time.SSP_start_time, LfootPrints(Yaw, LFoot_step_num - 1)); // init_sec
        ...
        //         ////////////////////////////////////////////////////////// compensate
        //         ////////////////////////////////////////////////////////// compensate
        //         joint[RHR].compensate_refAngle = cosWave(-hip_compensate, walking.time.SSP_time/2, walking.time.sec-walking.time.SSP_start_time, 0);
        //         std::cout<<"joint[RHR].compensate_refAngle == "<< joint[RHR].compensate_refAngle <<std::endl;

        //         d_L_offset_Z = LfootPrints(Z,currentLfootstep)-walking.step.LastLfoot(Z);
        // //        std::cout<<"d_L_offset_Z == "<< d_L_offset_Z <<std::endl;
        //         if(d_L_offset_Z > 0)
        //         {
        //             if(walking.time.sec < walking.time.periodTime / 2. + tasktime)
        //                 L_offset_Z = cosWave(d_L_offset_Z, walking.time.SSP_time/2, walking.time.sec - walking.time.SSP_start_time, walking.step.LastLfoot(Z));
        //         }
        //         else if(d_L_offset_Z < 0)
        //         {
        //             if(walking.time.sec > walking.time.periodTime / 2.)
        //                 L_offset_Z = cosWave(d_L_offset_Z, walking.time.SSP_time/2, walking.time.sec - (walking.time.SSP_start_time + walking.time.SSP_time/2), walking.step.LastLfoot(Z));
        //         }

        // //        std::cout<<"L_offset_Z == "<< L_offset_Z <<std::endl;

        // //        LFoot.refpos(Z)  = cosWave(walking.step.footHeight + walking.Landing.L_offset_landing, walking.time.SSP_time/2, walking.time.sec-walking.time.SSP_start_time, 0)+L_offset_Z;
        //     //    LFoot.refpos(Z)  = cosWave(walking.step.footHeight, walking.time.SSP_time/2, walking.time.sec-walking.time.SSP_start_time, walking.Landing.L_old_offset_landing);
        ...
        // printf("\n BezierCurve period : %lf", walking.time.SSP_time);
        // printf("\n BezierCurve time : %lf \n", predicted_walking_time - walking.time.SSP_start_time);
    ...
    else if (walking.Pattern_SP == walking.LSSP)
    {
        ...
        else if (walking.time.init_sec < walking.time.SSP_end_time) // 0.48
        {
            ...
            if (RFoot_step_num == 0)
            {
                ...
                RFoot.refpos(Yaw) = cosWave(RfootPrints(Yaw, RFoot_step_num) - walking.step.LastRfoot(Yaw), walking.time.SSP_time, walking.time.init_sec - walking.time.SSP_start_time, walking.step.LastRfoot(Yaw)); // init_sec
            }
            else
            {
                ...
                RFoot.refpos(Yaw) = cosWave(RfootPrints(Yaw, RFoot_step_num) - RfootPrints(Yaw, RFoot_step_num - 1), walking.time.SSP_time, walking.time.init_sec - walking.time.SSP_start_time, RfootPrints(Yaw, RFoot_step_num - 1)); // init_sec
            }
        }
        else
        {
        ...
        //         ////////////////////////////////////////////////////////////////// compensate
        //         cout << "\n == COMPENSATE_REFANGLE == \n" << endl;
        //         joint[LHR].compensate_refAngle = cosWave(hip_compensate, walking.time.SSP_time/2, walking.time.sec-walking.time.SSP_start_time, 0);
        //         std::cout<<"joint[LHR].compensate_refAngle == "<< joint[LHR].compensate_refAngle <<std::endl;

        //         d_R_offset_Z = RfootPrints(Z,currentRfootstep)-walking.step.LastRfoot(Z);
        //         if(d_R_offset_Z > 0)
        //         {
        //             if(walking.time.sec < walking.time.periodTime/2. + tasktime)
        //                 R_offset_Z = cosWave(d_R_offset_Z, walking.time.SSP_time/2, walking.time.sec - walking.time.SSP_start_time, walking.step.LastRfoot(Z));
        //         }
        //         else if(d_R_offset_Z < 0)
        //         {
        //             if(walking.time.sec > walking.time.periodTime / 2.)
        //                 R_offset_Z = cosWave(d_R_offset_Z, walking.time.SSP_time/2, walking.time.sec - (walking.time.SSP_start_time + walking.time.SSP_time/2), walking.step.LastRfoot(Z));
        //         }

        // //        RFoot.refpos(Z)  = cosWave(walking.step.footHeight + walking.Landing.R_offset_landing, walking.time.SSP_time/2, walking.time.sec-walking.time.SSP_start_time, 0)+R_offset_Z;
        // //        RFoot.refpos(Z)  = cosWave(walking.step.footHeight, walking.time.SSP_time/2, walking.time.sec-walking.time.SSP_start_time, walking.Landing.R_old_offset_landing);
    ...
    else if (zmp.previewControl.count < zmp.previewControl.RefTotalTrajSize * (zmp.previewControl.PreviewReadyTime + (walking.step.total_num) * walking.time.periodTime) / zmp.previewControl.RefTotalTime)
    {
        ...
        if (walking.time.wsec < walking.time.periodTime) // wsec
        {

            if (walking.time.wsec < walking.time.SSP_start_time) // wsec
            {
                walking.walk_SP = walking.DSP;
            }
            else if (walking.time.wsec < walking.time.SSP_end_time) // wsec
            {
                ...
                // if(walking.time.sec >= (walking.time.periodTime)/2-tasktime)
                // {
                //     if(walking.step.cur_num < walking.step.total_num-1)
                //         adjust_CoMHeight = cosWave(-25, walking.time.SSP_time/2, walking.time.sec-(walking.time.periodTime)/2-tasktime, 0);
                // }
                // else
                // {
                //     if(walking.step.cur_num > 0)
                //         adjust_CoMHeight  = cosWave(25, walking.time.SSP_time/2, walking.time.sec-walking.time.SSP_start_time, -25);
                // }
                // ////////////////////////////////////////////// compensate
                // ////////////////////////////////////////////// compensate

                if (walking.time.wsec >= (walking.time.periodTime) / 2 - tasktime && walking.time.wsec < (walking.time.periodTime) / 2) // wsec >= (0.2499  /  0.4999) && wsec < (0.25  /  0.5)
                {
            ...
            if (SSPMODE == false)
            {
                walking.time.wsec += (double)tasktime; // wsec
            }
            ...
            if (walking.time.wsec >= walking.time.periodTime - 0.001) // wsec
            {
                checkstep = false;
                walking.time.wsec = 0; // wsec
                ...
                if ((CommandFlag == ACT_INF_WALK || CommandFlag == ACT_STOPPING_WALK) && zmp.previewControl.count > zmp.previewControl.RefTotalTrajSize * (zmp.previewControl.PreviewReadyTime + (2) * walking.time.periodTime) / zmp.previewControl.RefTotalTime)
                {
                    ...
                    walking.step.cur_num--;
                    zmp.previewControl.count = (int)(zmp.previewControl.RefTotalTrajSize * (zmp.previewControl.PreviewReadyTime + (2) * walking.time.periodTime) / zmp.previewControl.RefTotalTime + 0.001) - 1; // 17000 * (2 + 2*1.5)/17 == 5000
...
void CKubot::hurdle(float hurdleTime)
{

    // static float HDLtime = 0;
    static float currentAngle[12]; // Now nDoF = 20
    VectorXd GBS_Vector(6);
    VectorXd GFL_Vector(6);
    VectorXd GFR_Vector(6);

    VectorXd IK_hurdleAngle(12);

    if (HDLtime >= 0 && standup_1 == false)
    { // zmp 이동

        GBS_Vector << 0.03, 0.05, 0.40, 0, 0, 0;
        GFR_Vector << 0, -0.05, 0, 0, 0, 0;
        GFL_Vector << 0, 0.05, 0, 0, 0, 0;

        IK_hurdleAngle << Geometric_IK_L(GBS_Vector, GFL_Vector), Geometric_IK_R(GBS_Vector, GFR_Vector);

        for (int j = 0; j < 12; j++)
        {
            hurdleAngle[j] = IK_hurdleAngle(j);
        }

        // hurdleAngle[12] = (30 *D2R);     // L_shoulder_pitch_joint 플러스가 어깨 뒤로
        // hurdleAngle[13] = (15 *D2R);     // L_elbow_roll_joint 플러스가 좌우로 나란히
        // hurdleAngle[14] = (-130 *D2R);     // L_hand_pitch_joint  플러스가 안으로 굽
        // hurdleAngle[15] = (30  *D2R);     // R_shoulder_pitch_joint
        // hurdleAngle[16] = (-15 *D2R);     // R_elbow_roll_joint
        // hurdleAngle[17] = (-130 *D2R);     // R_hand_pitch_joint
        // hurdleAngle[18] = (0 *D2R);     // Neck_yaw_joint
        // hurdleAngle[19] = (0 *D2R);     // head_pitch_joint
        ...
        if (HDLtime < hurdleTime && standup_1 == false)
        {
            for (int j = 0; j < 12; j++)
            {
                refAngle[j] = cosWave(hurdleAngle[j] - currentAngle[j], hurdleTime, HDLtime, currentAngle[j]);
            }
            HDLtime += tasktime;
        }
    ...
    else if (HDLtime >= 0 && standup_2 == false)
    { // 발 들기

        GBS_Vector << 0.03, 0.05, 0.40, 0, 0, 0;
        GFR_Vector << 0, -0.05, 0.07, 0, 0, 0;
        GFL_Vector << 0, 0.05, 0, 0, 0, 0;
        ...
        if (HDLtime < hurdleTime && standup_2 == false)
        {
            for (int j = 0; j < 12; j++)
            {
                refAngle[j] = cosWave(hurdleAngle[j] - currentAngle[j], hurdleTime, HDLtime, currentAngle[j]);
    ...
    else if (HDLtime >= 0 && standup_3 == false)
    { // 발 앞으로

        GBS_Vector << 0.03, 0.05, 0.40, 0, 0, 0;
        GFR_Vector << 0.16, -0.05, 0.08, 0, 0, 0;
        GFL_Vector << 0, 0.05, 0, 0, 0, 0;
        ...
        if (HDLtime < hurdleTime && standup_3 == false)
        {
            for (int j = 0; j < 12; j++)
            {
                refAngle[j] = cosWave(hurdleAngle[j] - currentAngle[j], hurdleTime, HDLtime, currentAngle[j]);
    ...
    else if (HDLtime >= 0 && standup_4 == false)
    { // 발 내리기

        GBS_Vector << 0.04, 0.05, 0.40, 0, 0, 0;
        GFR_Vector << 0.18, -0.05, 0, 0, 0, 0;
        GFL_Vector << 0, 0.05, 0, 0, 0, 0;
        ...
        if (HDLtime < hurdleTime && standup_4 == false)
        {
            for (int j = 0; j < 12; j++)
            {
                refAngle[j] = cosWave(hurdleAngle[j] - currentAngle[j], hurdleTime, HDLtime, currentAngle[j]);
    ...
    else if (HDLtime >= 0 && standup_5 == false)
    { // zmp 이동

        GBS_Vector << 0.15, -0.05, 0.40, 0, 0, 0;
        GFR_Vector << 0.18, -0.05, 0, 0, 0, 0;
        GFL_Vector << 0, 0.05, 0, 0, 0, 0;
        ...
        if (HDLtime < hurdleTime && standup_5 == false)
        {
            for (int j = 0; j < 12; j++)
            {
                refAngle[j] = cosWave(hurdleAngle[j] - currentAngle[j], hurdleTime, HDLtime, currentAngle[j]);
    ...
    else if (HDLtime >= 0 && standup_6 == false)
    { // 발 들기

        GBS_Vector << 0.16, -0.05, 0.40, 0, 0, 0;
        GFR_Vector << 0.18, -0.05, 0, 0, 0, 0;
        GFL_Vector << 0, 0.05, 0.08, 0, 0, 0;
        ...
        if (HDLtime < hurdleTime && standup_6 == false)
        {
            for (int j = 0; j < 12; j++)
            {
                refAngle[j] = cosWave(hurdleAngle[j] - currentAngle[j], hurdleTime, HDLtime, currentAngle[j]);
    ...
    else if (HDLtime >= 0 && standup_7 == false)
    { // 발 앞으로

        GBS_Vector << 0.17, -0.05, 0.40, 0, 0, 0;
        GFR_Vector << 0.18, -0.05, 0, 0, 0, 0;
        GFL_Vector << 0.18, 0.05, 0.08, 0, 0, 0;
        ...
        if (HDLtime < hurdleTime && standup_7 == false)
        {
            for (int j = 0; j < 12; j++)
            {
                refAngle[j] = cosWave(hurdleAngle[j] - currentAngle[j], hurdleTime, HDLtime, currentAngle[j]);
    ...
    else if (HDLtime >= 0 && standup_8 == false)
    { // 발 내리기

        GBS_Vector << 0.18, -0.05, 0.40, 0, 0, 0;
        GFR_Vector << 0.18, -0.05, 0, 0, 0, 0;
        GFL_Vector << 0.18, 0.05, 0, 0, 0, 0;
        ...
        if (HDLtime < hurdleTime && standup_8 == false)
        {
            for (int j = 0; j < 12; j++)
            {
                refAngle[j] = cosWave(hurdleAngle[j] - currentAngle[j], hurdleTime, HDLtime, currentAngle[j]);
    ...
    ///////////////////////////////////////////////////////////
    else if (HDLtime >= 0 && standup_9 == false)
    { // zmp 이동하면서 ready pose로

        GBS_Vector << 0.18, 0, 0.43, 0, 0, 0;
        GFR_Vector << 0.18, -0.05, 0, 0, 0, 0;
        GFL_Vector << 0.18, 0.05, 0, 0, 0, 0;
        ...
        if (HDLtime < hurdleTime && standup_9 == false)
        {
            for (int j = 0; j < 12; j++)
            {
                refAngle[j] = cosWave(hurdleAngle[j] - currentAngle[j], hurdleTime, HDLtime, currentAngle[j]);
